...
//search product
const searchProduct = (array,search) =>{
 const result = array.filter((product)=>{
   const  catagory = product.category.toLowerCase();
   const title = product.title.toLowerCase();
   const searchv = search.toLowerCase();
   return catagory.includes(searchv) || title.includes(searchv);
 });
 return result;
}
//
const filter = data => {
  document.getElementById('search-btn').addEventListener("click",()=>{
    ...
    if(value !== ""){
      const searcitem = searchProduct(data,value)
      showProducts (searcitem);
    }
...
const showProducts = (products) => {
 if(products.length == 0){
  document.getElementById("all-products").innerHTML="<h2 style='text-align: center'>No Result Found</h2>";
 }else{
